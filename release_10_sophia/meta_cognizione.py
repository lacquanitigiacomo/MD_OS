...
class MetaCognizione:
    ...
    BIAS_CATALOG = {
        ...
        "framing": "Interpretazione influenzata dalla presentazione del problema",
        "groupthink": "Conformita al gruppo a scapito del giudizio critico",
        "dunning_kruger": "Incompetenza meta-cognitiva che genera sovrastima",
        "hindsight": "Dopo l'evento, ritenere il risultato prevedibile",
        "self_serving": "Attribuire successi internamente e fallimenti esternamente",
        "false_consensus": "Sovrastimare quanto altri condividano le proprie opinioni",
        "optimism": "Tendenza a sottostimare rischi e sovrastimare benefici",
        "status_quo": "Preferenza per lo stato attuale rispetto al cambiamento",
        "loss_aversion": "Attribuire maggiore peso alle perdite che ai guadagni equivalenti"
    }
    ...
    def rifletti(self, query: str, ragionamento: str, passaggi: List[str]) -> Dict[str, Any]:
        try:
            ...
            logger.error("Errore DB in rifletti: %s", e)
    ...
    def _rileva_bias(self, ragionamento: str, passaggi: List[str]) -> List[Dict[str, Any]]:
        ...
        rilevati = []
        if "sicuramente" in testo_completo or "certamente" in testo_completo or "senza dubbio" in testo_completo:
            rilevati.append({"tipo": "overconfidence", "severita": "media",
                             "descrizione": self.BIAS_CATALOG["overconfidence"]})
        if "sempre" in testo_completo or "mai" in testo_completo or "tutti" in testo_completo:
            rilevati.append({"tipo": "availability", "severita": "bassa",
                             "descrizione": self.BIAS_CATALOG["availability"]})
        if "proprio come l'ultima volta" in testo_completo or "recentemente" in testo_completo or "ultimo" in testo_completo:
            rilevati.append({"tipo": "recency", "severita": "media",
                             "descrizione": self.BIAS_CATALOG["recency"]})
        if "dato che ho gia" in testo_completo or "investito troppo" in testo_completo or "non posso abbandonare" in testo_completo:
        ...
        if "come tutti sanno" in testo_completo or "ovvio che" in testo_completo:
            rilevati.append({"tipo": "false_consensus", "severita": "bassa",
                             "descrizione": self.BIAS_CATALOG["false_consensus"]})
        if "lo sapevo" in testo_completo or "l'avevo detto" in testo_completo:
            rilevati.append({"tipo": "hindsight", "severita": "bassa",
                             "descrizione": self.BIAS_CATALOG["hindsight"]})
        if "sono bravo" in testo_completo or "colpa loro" in testo_completo:
            rilevati.append({"tipo": "self_serving", "severita": "media",
                             "descrizione": self.BIAS_CATALOG["self_serving"]})
        return rilevati
    ...
    def _genera_correzione(self, bias: List[Dict]) -> str:
        ...
        for b in bias:
            ...
            elif b["tipo"] == "false_consensus":
                suggerimenti.append("Verificare dati empirici sulle opinioni altrui.")
            elif b["tipo"] == "hindsight":
                suggerimenti.append("Ricordare lo stato di incertezza pre-evento.")
            elif b["tipo"] == "self_serving":
                suggerimenti.append("Valutare criticamente proprie responsabilita.")
            else:
    ...
    def storico_bias(self, limit: int = 20) -> List[Dict[str, Any]]:
        try:
            ...
            logger.error("Errore DB in storico_bias: %s", e)
            return []
